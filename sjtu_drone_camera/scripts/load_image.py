...
### Albumentacja (ręczne odtworzenie działania biblioteki albumentatnion) ###
...
class RandomMotionBlur(nn.Module):
    ...
    def forward(self, sample):
        ...
        img_f = img.to(torch.float32).unsqueeze(0)  # [B,C,H,W]
        k2d = k2d.expand(img_f.shape[1], 1, k, k)   # [C,B,k,k]
...
def main():
    parser = argparse.ArgumentParser(description="Albumentacja (odtworzenie biblioteki albumentatnion)")
    parser.add_argument("--image", required=True, type=str)
    parser.add_argument("--bbox", default="false", choices=["true","false"], help="true/false")

    args = parser.parse_args()

    if not os.path.isfile(args.image):
        print(f"Brak pliku obrazu: {args.image}")
        raise SystemExit(2)
    ...
    bbox = None
    if args.bbox == "true":
        print('przyszłe odczytanie bbox todo')
    else:
    ...
    class_id = 0 ## NUMER KLASY

    label_line = f"{class_id} {cx:.6f} {cy:.6f} {w:.6f} {h:.6f}"
    print("YOLO label:", label_line)


    # Generowanie albumu z jednego obrazu + bbox (YOLO cx,cy,w,h)
